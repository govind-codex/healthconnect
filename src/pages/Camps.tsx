...
import { 
  fadeInUp, staggerContainer, staggerItem, staggerItemScale,
  useCountUp
} from '../utils/useAnimations';

const CampCard: React.FC<{ camp: HealthCamp; onClick: () => void; index: number }> = ({ camp, onClick, index }) => {
  return (
    <motion.div
      layout
        ...
        <div className="relative h-56 rounded-[28px] overflow-hidden bg-slate-200">
          ...
          <div className="absolute top-4 left-4 z-20 flex gap-2">
            <motion.span
              ...
              className="bg-green-500 text-white px-3 py-1 rounded-full text-[9px] font-bold uppercase tracking-wider shadow-lg"
            >
              Approved
            </motion.span>
          ...
          <div className="absolute bottom-4 left-4 z-20 pr-4">
            ...
            <h3 className="text-white text-2xl font-black tracking-tight leading-tight group-hover:text-blue-400 transition-colors duration-300 line-clamp-2">
              {camp.name}
            </h3>
          </div>
        </div>
      ...
      <div className="p-6 flex flex-col flex-grow">
        ...
        <div className="flex gap-2">
          <motion.button
            ...
            whileTap={{ scale: 0.97 }}
            onClick={onClick}
            className="flex-1 geometric-btn-primary py-3.5 shadow-lg shadow-blue-200/30"
          >
            View Details
          </motion.button>
          <motion.a
...
const Camps = () => {
  ...
  const ActiveCountRef = () => {
    const [ref, count] = useCountUp(12, 1500);
    return <p ref={ref as React.Ref<HTMLParagraphElement>} className="text-2xl sm:text-3xl font-black text-slate-900 tracking-tighter">{count}</p>;
  };
    ...
    <div className="min-h-screen pt-8 sm:pt-12 pb-24">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <motion.div
          ...
          initial="hidden"
          animate="visible"
          className="flex flex-col lg:flex-row lg:items-end justify-between mb-12 sm:mb-16 gap-8 text-left"
        >
          ...
          <motion.div variants={staggerItem} className="shrink-0 flex gap-6 sm:gap-8 border-l border-slate-200 pl-6 sm:pl-8">
            <div>
              <ActiveCountRef />
        ...
        <motion.div
          ...
          className={cn(
            "bg-white p-4 sm:p-6 rounded-[2rem] sm:rounded-[2.5rem] shadow-xl border border-slate-100 mb-8 sm:mb-12 transition-all duration-500",
            isSearchFocused ? "shadow-2xl shadow-blue-100/50 border-blue-200/50" : "shadow-xl shadow-slate-200/40"
          )}
        >
        ...
        {filteredCamps.length > 0 ? (
          ...
          >
            <motion.div
              ...
              className="bg-slate-100 w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-6"
            >
      ...
      <AnimatePresence>
        {selectedCamp && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
            ...
            >
              <div className="lg:w-2/5 h-64 lg:h-auto shrink-0 relative overflow-hidden">
                <motion.img
                  initial={{ scale: 1.1 }}
                  animate={{ scale: 1 }}
                ...
                <button 
                  onClick={() => setSelectedCamp(null)}
                  className="absolute top-6 left-6 lg:hidden bg-white/90 backdrop-blur-sm p-2 rounded-full shadow-lg hover:scale-110 transition-transform"
                >
              ...
              <motion.div
                ...
                initial="hidden"
                animate="visible"
                className="p-6 sm:p-8 lg:p-12 flex-1 relative"
              >
                <button 
                  onClick={() => setSelectedCamp(null)}
                  className="absolute top-8 right-8 hidden lg:flex bg-slate-100 p-2 rounded-full hover:bg-slate-200 hover:rotate-90 transition-all duration-300"
                >
                  <ChevronRight className="h-6 w-6" />
                </button>

                <motion.div variants={staggerItem} className="bg-blue-50 text-blue-700 text-[10px] font-bold uppercase tracking-widest px-3 py-1 rounded-full w-fit mb-4">
                  Free Medical Camp
                </motion.div>
                
                <motion.h2 variants={staggerItem} className="text-3xl font-extrabold text-slate-900 mb-2">{selectedCamp.name}</motion.h2>
                <motion.p variants={staggerItem} className="text-lg font-bold text-blue-600 mb-8">{selectedCamp.organizerName}</motion.p>
                ...
                <motion.div variants={staggerItem} className="mb-8">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selectedCamp.availableTests.map((test, i) => (
                      <motion.div
                        key={test}
                        initial={{ opacity: 0, scale: 0.8 }}
                        animate={{ opacity: 1, scale: 1 }}
                        transition={{ delay: i * 0.08 }}
                        className="flex items-center px-4 py-2 bg-green-50 text-green-700 rounded-xl text-xs font-bold border border-green-100"
                      >
                ...
                <motion.div variants={staggerItem} className="mb-8 overflow-hidden rounded-[2rem] border border-slate-100 shadow-sm">
                ...
                <motion.div variants={staggerItem} className="flex items-center justify-between pt-6 border-t border-slate-100">
